...
# DeepSeek 클라이언트 초기화
client = OpenAI(
    api_key="sk-9d2da94df409489c8cfd3673b509665b",  # 실제 키로 교체 필요
...
- **병음:** [전체 문장 병음]
- **해석:** [한국어 뜻]
- **📝 간체 변환:** [원문이 번체일 경우에만 추가, 간체로 변환한 문장]  
  (※ 원문이 이미 간체라면 이 항목은 생략합니다)

- **HSK 4급 이상 단어 분석 (한 글자씩 해부):**
  - [단어] (병음): 뜻 (HSK 급수)
    - (한자 분석) [첫째 글자(뜻)] + [둘째 글자(뜻)]  
  - [단어] (병음): 뜻 (HSK 급수)
    - (한자 분석) [첫째 글자(뜻)] + [둘째 글자(뜻)]
...
❗ **추가 규칙**:
- HSK 4급 이상의 단어만 추출하세요.
- 각 단어의 한 글자씩 분석을 반드시 포함하세요.
- 단어 분석은 가로로 정렬하지 말고, 위 예시처럼 **단어 → 병음 → 뜻 → 한 글자 분석** 순서로 세로로 나열하세요.
- 모든 문장에 대해 위 형식을 반복하세요.
- 문장과 문장 사이에는 반드시 빈 줄을 넣어서 시각적으로 분리하세요.
- 절대 표(테이블)를 사용하지 마세요.
