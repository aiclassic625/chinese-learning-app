...
- 각 단어의 한 글자씩 뜻 풀이만 제공하세요. (예: 休息 → 休: 쉬다, 息: 숨쉬다)
...
# 🚀 스트리밍 버전 (글자가 차차 나타나서 체감 속도가 훨씬 빠름)
def generate_study_material_stream(chinese_text):
    """
    스트리밍 방식으로 중국어 텍스트를 학습 자료로 변환합니다.
    """
    try:
        if not chinese_text or len(chinese_text.strip()) < 5:
            yield "텍스트가 너무 짧습니다. (최소 5자 이상 필요)"
            return
